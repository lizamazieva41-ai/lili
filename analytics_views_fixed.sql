-- Fixed analytics views for Phase 2 with correct column names
...
SELECT
  ...
  CASE
    ...
    THEN ROUND(
      (COUNT(DISTINCT ul.id) * 30.0 / NULLIF(EXTRACT(DAY FROM NOW() - MIN(DATE(ul."createdAt"))), 0)) /
...
GROUP BY l.id, l."userId", l.plan, DATE(ul."createdAt"), l.limits;
...
CREATE INDEX IF NOT EXISTS idx_campaign_stats_user ON campaign_stats_view("userId");
CREATE INDEX IF NOT EXISTS idx_message_status_time_campaign ON message_status_time_view("campaignId");
