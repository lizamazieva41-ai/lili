...
GROUP BY l.id, l.user_id, l.plan, DATE(ul.created_at), l.limits;
...
SELECT
  ...
  COUNT(CASE WHEN ml.status = 'READ' THEN 1 END) as read,
  COUNT(CASE WHEN ml.status = 'FAILED' THEN 1 END) as failed,
  ROUND(
    CASE
      WHEN COUNT(ml.id) > 0
      THEN (COUNT(CASE WHEN ml.status IN ('DELIVERED', 'READ') THEN 1 END) * 100.0 / COUNT(ml.id))
      ELSE 0
    END, 2
  ) as success_rate,
  ROUND(
    CASE
      WHEN COUNT(ml.id) > 0
      THEN (COUNT(CASE WHEN ml.status = 'FAILED' THEN 1 END) * 100.0 / COUNT(ml.id))
      ELSE 0
    END, 2
  ) as error_rate,
  AVG(EXTRACT(EPOCH FROM (ml.delivered_at - ml.sent_at))) as avg_delivery_time_seconds
FROM campaigns c
...
SELECT
  ...
  ml.delivered_at,
  ml.read_at,
  EXTRACT(EPOCH FROM (ml.delivered_at - ml.sent_at)) as sent_to_delivered_seconds,
  EXTRACT(EPOCH FROM (ml.read_at - ml.delivered_at)) as delivered_to_read_seconds,
  EXTRACT(EPOCH FROM (ml.read_at - ml.sent_at)) as total_response_time_seconds
...
SELECT
  ...
  COUNT(CASE WHEN j.status = 'COMPLETED' AND j.updated_at >= CURRENT_DATE THEN 1 END) as completed_today,
  COUNT(CASE WHEN j.status = 'FAILED' AND j.updated_at >= CURRENT_DATE THEN 1 END) as failed_today,
  AVG(EXTRACT(EPOCH FROM (j.updated_at - j.created_at))) as avg_processing_time_seconds,
  MAX(EXTRACT(EPOCH FROM (j.updated_at - j.created_at))) as max_processing_time_seconds
FROM queues q
...
SELECT
  ...
  AVG(EXTRACT(EPOCH FROM (j.updated_at - j.created_at))) as avg_duration_seconds,
  MAX(EXTRACT(EPOCH FROM (j.updated_at - j.created_at))) as max_duration_seconds
FROM jobs j
...
SELECT
  ...
  array_agg(DISTINCT al.user_id) as affected_users,
...
GROUP BY DATE(al.created_at), al.ip_address, al.user_agent
...
WHERE al.severity IN ('WARNING', 'ERROR', 'CRITICAL')
  AND al.created_at >= CURRENT_DATE - INTERVAL '30 days'
GROUP BY DATE(al.created_at), al.severity
ORDER BY date DESC, al.severity;

-- Create indexes for performance
CREATE INDEX idx_license_daily_usage_date ON license_daily_usage_view(date);
CREATE INDEX idx_license_daily_usage_license ON license_daily_usage_view(license_id);
CREATE INDEX idx_campaign_stats_campaign ON campaign_stats_view(campaign_id);
CREATE INDEX idx_campaign_stats_user ON campaign_stats_view(user_id);
CREATE INDEX idx_message_status_time_campaign ON message_status_time_view(campaign_id);
CREATE INDEX idx_queue_stats_queue ON queue_realtime_stats_view(queue_name);
CREATE INDEX idx_job_failure_type ON job_type_failure_view(job_type);
CREATE INDEX idx_auth_failures_date ON auth_failures_view(date);
CREATE INDEX idx_security_events_date ON security_events_daily_view(date);
